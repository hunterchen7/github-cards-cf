// Theme system ported from vn7n24fzkq/github-profile-summary-cards (MIT).
// Each theme is a 7-tuple of colors + a stroke opacity. `github_dark` matches
// the card in the reference embed.
...
// name -> new Theme('title','text','background','stroke', strokeOpacity, 'icon','chart')
...
// Resolve an arbitrary user-supplied theme name to a guaranteed-existing one.
export function resolveThemeName(themeName: string | null | undefined): string {
  return themeName && ThemeMap.has(themeName) ? themeName : FALLBACK_THEME_NAME;
}
...
// Anything else is ignored so user-supplied colors can't inject content into SVG/CSS.
...
export function parseThemeColorOverride(query: URLSearchParams): ThemeColorOverride {
  return {
    title: sanitizeHexColor(query.get('title_color')),
    text: sanitizeHexColor(query.get('text_color')),
    background: sanitizeHexColor(query.get('bg_color')),
    border: sanitizeHexColor(query.get('border_color')),
    icon: sanitizeHexColor(query.get('icon_color')),
    chart: sanitizeHexColor(query.get('chart_color')),
  };
}
...
export function resolveTheme(themeName: string | null | undefined, override?: ThemeColorOverride): Theme {
